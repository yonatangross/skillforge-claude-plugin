...
Usage: ./get-events.py [--event-id "evt_abc123"]
"""
import argparse
import json
import sys
...
def main():
    parser = argparse.ArgumentParser(description="Get mem0 events")
    parser.add_argument("--event-id", help="Specific event ID (optional)")
    ...
    try:
        ...
        if args.event_id:
            result = client.get_event(event_id=args.event_id)
        else:
