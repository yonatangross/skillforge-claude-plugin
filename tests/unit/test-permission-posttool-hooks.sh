...
test_write_headers_skips_json_files() {
    ...
    # JSON files should NOT have OrchestKit header added to content
    # The hook should pass through the original content
    assert_contains "$output" '"continue"'

    # Check the output content doesn't include OrchestKit comment in the actual content value
    local content_value
    content_value=$(echo "$output" | jq -r '.hookSpecificOutput.updatedInput.content // ""' 2>/dev/null)
    if [[ "$content_value" == *"Generated by OrchestKit"* ]]; then
        fail "JSON files should not have OrchestKit header"
    fi
}
...
test_write_headers_skips_files_with_skillforge_marker() {
    ...
    # Check content doesn't have duplicate OrchestKit markers
    local content_value
    content_value=$(echo "$output" | jq -r '.hookSpecificOutput.updatedInput.content // ""' 2>/dev/null)
    local marker_count
    marker_count=$(echo "$content_value" | grep -c "OrchestKit" || echo "0")

    if [[ "$marker_count" -gt 1 ]]; then
        fail "Should not add duplicate OrchestKit header"
    fi
}
...
test_write_headers_skips_unknown_extensions() {
    ...
    local content_value
    content_value=$(echo "$output" | jq -r '.hookSpecificOutput.updatedInput.content // ""' 2>/dev/null)
    if [[ "$content_value" == *"OrchestKit"* ]]; then
        fail "Unknown extensions should not get OrchestKit header"
    fi
}
